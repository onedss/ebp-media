...
unsigned RTSPClient::sendDescribeCommand(responseHandler* responseHandler, Authenticator* authenticator) {
    // 添加sdp后缀
    size_t len = strlen(fBaseURL);
    if(strcasecmp(fBaseURL + len - 4, ".sdp") != 0) {
        len += 5;
        char *url = new char[len];
        snprintf(url, len, "%s.sdp", fBaseURL);
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  if (socketNumToServer >= 0) {
    ...
    envir().taskScheduler().setBackgroundHandling(fInputSocketNum, SOCKET_READABLE, (TaskScheduler::BackgroundHandlerProc*)&incomingDataHandler, this);
  }
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
  ...
  if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) {
    ...
    if (err == EINPROGRESS || err == EWOULDBLOCK) {
      ...
      envir().taskScheduler().setBackgroundHandling(socketNum, SOCKET_WRITABLE, (TaskScheduler::BackgroundHandlerProc*)&connectionHandler, this);
      return 0;
    }
