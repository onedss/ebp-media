...
Boolean RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize) {
  ...
  if (fGS != NULL && !fGS->output(envir(), packet, packetSize)) success = False;
...
::startNetworkReading(TaskScheduler::BackgroundHandlerProc* handlerProc) {
  // Normal case: Arrange to read UDP packets:
	if(fGS != NULL) envir().taskScheduler().turnOnBackgroundReadHandling(fGS->socketNum(), handlerProc, fOwner);
...
Boolean RTPInterface::handleRead(unsigned char* buffer, unsigned bufferMaxSize,
  ...
  if (fNextTCPReadStreamSocketNum < 0) {
    ...
    tcpSocketNum = -1;
    readSuccess = fGS != NULL ? fGS->handleRead(buffer, bufferMaxSize, bytesRead, fromAddress) : false;
  } else {
...
void SocketDescriptor::registerRTPInterface(unsigned char streamChannelId,
  ...
  if (isFirstRegistration) {
    ...
    fEnv.taskScheduler().setBackgroundHandling(fOurSocketNum, SOCKET_READABLE, handler, this);
  }
}
